...
%             disp(['Iteration ' num2str(iter) ': error is ' num2str(cost)]);
        ...
        if ~isempty(labels)
            groupColor = [0.7 0.7 0.7; 1 0.2 0.2; 0.2 0.2 1; 0.2 1 0.2];
            
            if no_dims == 1
                scatter(ydata, ydata, 9, groupColor(labels,:), 'filled');
            elseif no_dims == 2
                scatter(ydata(:,1), ydata(:,2), 9, groupColor(labels,:), 'filled');
            else
                scatter3(ydata(:,1), ydata(:,2), ydata(:,3), 40, groupColor(labels,:), 'filled');
            end
            axis equal tight
            set(gca,'color','k')
%             axis off
