function resultingP = loadData_v4(inFolder,outFolder,envSize,doWindowed)
    ...
    allPaths = getFilePaths(inFolder,'notes.csv');
...
%         mkStablizedVideos(allPaths{i}(1:find(ismember(allPaths{i},'/'),1,'last')-1));
        ...
        [s.pos.p s.pos.uninterp] = trackPosition_1LED_v4(allPaths{i}(1:find(ismember(allPaths{i},'/'),1,'last')-1));

        fprintf(['\n\n\tProcessing calcium from:  ' allPaths{i}(1:find(ismember(allPaths{i},'/'),1,'last')-1) '\n'])
        ms = msRunUpdated_v4(allPaths{i}(1:find(ismember(allPaths{i},'/'),1,'last')-1),doWindowed);
        s.calcium = ms;
...
%         [s.pos.p s.pos.uninterp] = trackPosition_1LED(allPaths{i}(1:find(ismember(allPaths{i},'/'),1,'last')-1));
        ...
        s = alignTraceData_v4(allPaths{i}(1:find(ismember(allPaths{i},'/'),1,'last')-1),s);
