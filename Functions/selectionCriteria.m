function selectionCriteria(paths,varargin)
    ...
    for p = paths'
        ...
        if ismember({'wholemap_si'},varargin) || ismember({'wholemap_unmatched_si'},varargin)

            if isfield(s.processed,'splithalf') && isfield(s.processed.splithalf,'wholemap_unmatched_si')
%                 continue
            end
