function ms = msRunUpdated_v4(doPath,doWindowed)
...
%     cleanCalciumVideos(doPath)
    ...
    ms = msGenerateVideoObj_v4([doPath '/V4c'],'');
    ...
    slind = find(ismember(ms.dirName,'/\'));
    normCorrPath = getFilePaths(ms.dirName(1:slind(end)-1),'msvideo.avi');

    if length(normCorrPath) > 1
        for k = 1:length(normCorrPath)
            delete(normCorrPath{k});
        end
        normCorrPath = [];
    end
    
    %% 2 - Perform motion correction using NormCorre
    if isempty(normCorrPath)
        ...
        fprintf('\n\t\t--Prior motion-corrected video found');
        slind = find(ismember(normCorrPath{1},'/\'));
        ms.dirName = normCorrPath{1}(1:slind(end)-1);
    end
    
    interpNormCorreVid(ms);
