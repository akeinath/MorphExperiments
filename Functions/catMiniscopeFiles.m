function catMiniscopeFiles(inFolder,outFolder)
    if nargin<2
...
%             v = VideoReader([path(1:slashInds(end)) bv{bvi}]);
%             ov = VideoWriter([outFolder '/behavCam' num2str(bvi+currentBV)]);
%             allFrames = zeros([v.height v.width 3 v.NumberOfFrames],'uint8');
%             if bvi==1
%                 for bvj = 1:1000;
%                     thisFrame = read(v, bvj);
%                     allFrames(:,:,:,bvj) = thisFrame;
%                 end
%                 allFrames(:,:,:,og(find(og(1:15,1)==behavCam & abs(og(1:15,3))>500),2)) = [];
%             end
%             open(ov);
%             writeVideo(ov,allFrames);
%             close(ov)
        end
...
%             v = VideoReader([path(1:slashInds(end)) msv{msvi}]);
%             ov = VideoWriter([outFolder '/msCam' num2str(msvi+currentMsV)]);
%             allFrames = zeros([v.height v.width 3 v.NumberOfFrames],'uint8');
%             if msvi==1
%                 for bvj = 1:1000;
%                     thisFrame = read(v, bvj);
%                     allFrames(:,:,:,bvj) = thisFrame;
%                 end
%                 allFrames(:,:,:,og(find(og(1:15,1)==msCam & abs(og(1:15,3))>500),2)) = [];
%             end
%             open(ov);
%             writeVideo(ov,allFrames);
%             close(ov)
