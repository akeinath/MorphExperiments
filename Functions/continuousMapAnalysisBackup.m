...
%             mdsND(sim,envs,envLabel,[root '/MDS_nD/'],3);
...
%         while length(uclass(1,:)) > length(allClassXTime(1,:))
%             allClassXTime = [allClassXTime repmat({[]},[4 1])];
%         end
%         
%         for j = 1:length(uclass(1,:))
%             for i = 1:4
%                 allClassXTime{i,j} = [allClassXTime{i,j}; ...
%                     nansum(uclass(:,j)==(i-1))./nansum(~isnan(uclass(:,j)))];
%             end
%         end
    end
