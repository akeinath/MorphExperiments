function [dp1x dp2x dp1y dp2y] = fitDeformationPaths(p1,p2)
    ...
    for i = 0:nanmax([tp1f(1,:) tp2f(1,:)])
        Ay = [nanmin(tp1f(2,tp1f(1,:)==i)) nanmax(ceil(tp1(2,tp1f(1,:)==i)))];
        By = [nanmin(tp2f(2,tp2f(1,:)==i)) nanmax(ceil(tp2(2,tp2f(1,:)==i)))];
        boundsMapX = [boundsMapX; i Ay By];
    end
    
    for j = 0:nanmax([tp1f(2,:) tp2f(2,:)])
        Ax = [nanmin(tp1f(1,tp1f(2,:)==j)) nanmax(ceil(tp1(1,tp1f(2,:)==j)))];
        Bx = [nanmin(tp2f(1,tp2f(2,:)==j)) nanmax(ceil(tp2(1,tp2f(2,:)==j)))];
        boundsMapY = [boundsMapY; j Ax Bx];
    end
    
%     boundsMapX(end,2:5) = boundsMapX(end-1,2:5);
%     boundsMapY(end,2:5) = boundsMapY(end-1,2:5);
...
%     
%     tmp = [diff(boundsMapX(:,[2 3]),[],2) diff(boundsMapX(:,[4 5]),[],2)];
%     rescale = bsxfun(@rdivide,38,tmp);
%     
%     dp1x = p1;
%     [a b] = ismember(tp1f(1,:),boundsMapX(:,1));
%     dp1x(2,:) = (dp1x(2,:)- [boundsMapX(b,2)]').*rescale(b,1)' ;
%     
%     dp2x = p2;
%     [a b] = ismember(tp2f(1,:),boundsMapX(:,1));
%     dp2x(2,:) = (dp2x(2,:)- [boundsMapX(b,4)]').*rescale(b,2)' ;
%     
%     %%% Expand on X
%     
%     tmp = [diff(boundsMapY(:,[2 3]),[],2) diff(boundsMapY(:,[4 5]),[],2)];
%     rescale = bsxfun(@rdivide,37,tmp);
%     
%     dp1y = p1;
%     [a b] = ismember(tp1f(2,:),boundsMapY(:,1));
%     dp1y(1,:) = (dp1y(1,:)- [boundsMapY(b,2)]').*rescale(b,1)' ;
%     
%     dp2y = p2;
%     [a b] = ismember(tp2f(2,:),boundsMapY(:,1));
%     dp2y(1,:) = (dp2y(1,:)- [boundsMapY(b,4)]').*rescale(b,2)' ;
