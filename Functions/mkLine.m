function h = mkLine(d,xl,groupColor)    
...
%                 tmp(round(length(tmp).*0.5)) tmp(round(length(tmp).*0.75)) tmp(end)]';
            quarts(:,j) = [tmp(1) nanmean(tmp)-(nanstd(tmp))./sqrt(nansum(~isnan(tmp))) ...
                nanmean(tmp) nanmean(tmp)+(nanstd(tmp))./sqrt(nansum(~isnan(tmp))) tmp(end)]'; %./sqrt(nansum(~isnan(tmp)))
        end
...
%             'facecolor',groupColor(i,:),'edgealpha',0,'facealpha',0.1)
